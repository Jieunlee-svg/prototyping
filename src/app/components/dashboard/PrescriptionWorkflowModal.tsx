...
import {
  X, FileText, CheckCircle, Bell, Clock, Send, Pill, Info, Smartphone, Minus, Plus, ChevronRight, AlertCircle, Calendar as CalendarIcon, Settings, Loader2
} from 'lucide-react';
...
export const PrescriptionWorkflowModal: React.FC<PrescriptionWorkflowModalProps> = ({
  ...
  const validatePhone = (value: string) => {
    ...
    if (raw.length < 10 || raw.length > 11) return '올바른 핸드폰 번호 형식이 아닙니다. (예: 010-0000-0000)';
    if (!raw.startsWith('010') && !raw.startsWith('011') && !raw.startsWith('016') && !raw.startsWith('017') && !raw.startsWith('018') && !raw.startsWith('019')) return '올바른 핸드폰 번호 형식이 아닙니다.';
    return '';
  };

  const isPhoneValid = !validatePhone(patientPhone);
  const isSendable = isPhoneValid && message.trim().length > 0;
  ...
  // ── Focus to phone input when entering step 3 ──
  useEffect(() => {
    if (currentStep === 3) {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!isSendable || isSending) return;
    setIsSending(true);
    try {
      // Simulate API call
  ...
  const handlePhoneKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && isPhoneValid) nextStep();
  };
              ...
              {STEP_LABELS.map((label, i) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      'flex-1 py-2.5 text-center text-xs font-semibold border-b-[3px] transition-all focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-300',
              ...
              <div className="grid grid-cols-2 gap-5 h-full">
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden flex flex-col bg-gray-50 min-h-[420px]">
                  ...
                  <div className="flex-1 p-4 flex items-start justify-center overflow-auto">
                    {prescription.imageUrl ? (
                      <ImageWithFallback src={prescription.imageUrl} alt="처방전 원본 이미지" className="max-w-full rounded-lg shadow-md" />
                    ) : (
                      <div className="flex flex-col items-center justify-center text-gray-300 py-16">
                        ...
                        <p className="text-xs text-gray-300 mt-1">이미지가 업로드되지 않았습니다.</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden flex flex-col min-h-[420px]">
                  ...
                  <div className="flex-1 p-3 overflow-y-auto space-y-1">
                    {ocrData.drugs.length > 0 ? ocrData.drugs.map((drug, idx) => (
                      <div key={idx} className="flex items-start gap-3 p-3 border-b border-gray-100 last:border-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[13px] font-semibold text-gray-900 truncate">{drug.name}</div>
                      ...
                      <div className="flex flex-col items-center justify-center py-16 text-gray-300">
                        ...
                        <p className="text-sm text-gray-400">약물 정보를 불러오지 못했습니다.</p>
                        <p className="text-xs text-gray-300 mt-1">처방전 이미지를 다시 확인해 주세요.</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex flex-col gap-4 max-w-3xl mx-auto">
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden flex flex-col bg-white">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {allSmartTags.map(tag => (
                            <button
                              ...
                              className={clsx(
                                'px-3 py-1.5 text-xs rounded-full transition-all font-semibold border shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 active:scale-95',
                  ...
                  <textarea
                    ...
                    className="w-full p-5 bg-transparent resize-none outline-none text-sm leading-relaxed text-gray-800 placeholder:text-gray-300 min-h-[280px] focus:ring-2 focus:ring-blue-100 rounded-b-xl"
                    placeholder="약품을 추가하면 상담 내용이 자동으로 작성됩니다."
                  />
                  <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                    <span className="text-xs text-gray-300">상담 내용을 자유롭게 수정할 수 있습니다.</span>
              ...
              <div className="space-y-4 max-w-2xl mx-auto">
                ...
                <div className="border border-gray-200 rounded-xl p-5">
                  ...
                  <div className="flex items-start gap-3">
                    <label htmlFor="patient-phone" className="text-sm text-gray-500 min-w-[90px] pt-2.5">핸드폰 번호</label>
                    <div className="flex-1">
                      <input
                        ...
                        className={clsx(
                          'w-full px-3 py-2.5 border rounded-lg text-sm outline-none transition-all font-mono focus:ring-2',
                        ...
                        placeholder="010-0000-0000 (숫자 11자리)"
                        autoComplete="tel"
                      />
                      {phoneTouched && phoneError && (
                ...
                <div className="border border-gray-200 rounded-xl p-5 space-y-5">
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1 space-y-2">
                      ...
                      <button className="w-full flex items-center gap-2 px-3 py-2.5 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 active:scale-95 transition-all text-left focus:outline-none focus:ring-2 focus:ring-blue-300">
                    ...
                    <div className="w-[160px] space-y-2 flex-shrink-0">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="w-9 h-[42px] rounded-lg border border-gray-200 bg-white hover:bg-gray-50 active:scale-90 flex items-center justify-center transition-all text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-300"
                          aria-label="복약 기간 줄이기"
                        >
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-sm font-bold text-gray-800 flex items-center gap-1.5 cursor-pointer" onClick={() => setSendAppReminder(!sendAppReminder)}>
                        <Smartphone className="w-4 h-4 text-blue-500" />
                        웰체크 앱으로 설정 전송
                      </label>
                      <button
                        role="switch"
                        aria-checked={sendAppReminder}
                        className={clsx(
                          'relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-1',
                ...
                <div className="border border-gray-200 rounded-xl p-5">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <input
                      type="number"
                      value={durationDays}
                      className="w-20 text-center py-2 border border-gray-200 rounded-lg text-sm font-medium outline-none bg-gray-50 text-gray-400 cursor-not-allowed"
                      readOnly
                      aria-readonly="true"
                    />
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-2">
                  ...
                  <button className="text-xs text-blue-600 font-bold hover:underline flex items-center gap-0.5 ml-auto flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded">
              ...
              <div className="max-w-[560px] mx-auto">
                {isCompleted ? (
                  ...
                  <div className="text-center py-10">
                    <div className="w-[72px] h-[72px] rounded-full bg-emerald-50 flex items-center justify-center text-3xl mx-auto mb-4 animate-in zoom-in-75 duration-300">
                      ✅
                    </div>
                    <h2 className="text-xl font-bold text-gray-900 mb-2">전송 완료!</h2>
                    <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                      {prescription.patientName} 님께 복약 안내 메시지와<br />복약알림이 성공적으로 발송되었습니다.
                    </p>
                    <button
                      onClick={onClose}
                      className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-bold rounded-lg transition-all text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-2"
                    >
                      목록으로 돌아가기
                    </button>
                  </div>
                  ...
                  <form onSubmit={handleSend} className="space-y-5">
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                      {[
                        ...
                        <div key={key} className={clsx('flex items-center justify-between px-5 py-3 text-sm', i < 3 && 'border-b border-gray-100')}>
                          ...
                          <span className={clsx('font-semibold text-gray-800 text-right max-w-[300px] truncate', key === '수신자' && !patientPhone && 'text-red-400')}>
                            {value}
                          </span>
                        </div>
                      ))}
                    ...
                    {/* Phone warning if not set */}
                    {!isPhoneValid && (
                      <div className="flex items-center gap-2 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600">
                        <AlertCircle className="w-4 h-4 flex-shrink-0" />
                        <span>핸드폰 번호를 올바르게 입력해야 문자를 전송할 수 있습니다.</span>
                        <button
                          type="button"
                          onClick={() => goStep(3)}
                          className="ml-auto text-xs font-bold text-red-600 hover:underline whitespace-nowrap focus:outline-none"
                        >
                          3단계로 이동 →
                        </button>
                      </div>
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        onClick={prevStep}
                        disabled={isSending}
                        className="px-5 py-3.5 border border-gray-200 text-gray-600 font-medium rounded-lg hover:bg-gray-50 active:scale-95 transition-all whitespace-nowrap text-sm disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-gray-300"
                      >
                        ← 수정
                      </button>
                      <button
                        ...
                        className={clsx(
                          'flex-1 py-3.5 font-bold rounded-lg transition-all shadow-lg flex items-center justify-center gap-2 text-[15px] focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-2',
                          isSendable && !isSending
            ...
            <div className="px-7 py-4 border-t border-gray-200 flex items-center justify-between bg-white flex-shrink-0">
              ...
              <div className="flex gap-2.5">
                ...
                {currentStep < 4 && (
                  <button
                    onClick={nextStep}
                    className="px-5 py-2 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-sm font-semibold rounded-lg transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-1"
                  >
                    다음 단계 →
                  </button>
                )}
